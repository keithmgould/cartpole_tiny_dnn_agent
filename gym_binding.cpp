#include <boost/enable_shared_from_this.hpp> // remove me when done with simulator
...
#include <stdio.h>
#include <random>
#include <iostream> // remove me when done testing
...
int Space::weighted_sample(tiny_dnn::vec_t& prediction)
{
	float total = 0;
	tiny_dnn::vec_t::iterator it;
	...
	float random_float = dis(rand_generator);

	for (it = prediction.begin() ; it != prediction.end(); ++it)
	{
		if(random_float < *it) { return it - prediction.begin(); }
		random_float -= *it;
	}
...
public:
	...
	void observation_parse(const Json::Value& v, tiny_dnn::vec_t& save_here)
	{
		if (!v.isArray())
	...
	void step(const int action, bool render, State& save_state_here) override
	{
		Json::Value act_json;
		boost::shared_ptr<Space> aspace = action_space();

		act_json["action"] = action;
